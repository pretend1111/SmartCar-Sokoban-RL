...
net_path1 = "model_18_0.7639_quant.tflite"                                  # 定义模型的路径
labels = [line.rstrip() for line in open("/sd/labels_animal_fruits.txt")]   # 加载标签
net1 = tf.load(net_path1, load_to_fb=True)                                  # 加载模型

net_path2 = "model_18_0.7639_quant.tflite"                                  # 定义模型的路径
net2 = tf.load(net_path2, load_to_fb=True)                                  # 加载模型

net_path3 = "model_18_0.7639_quant.tflite"                                  # 定义模型的路径
...
while(True):
    ...
    for r in img.find_rects(threshold = 20000):             # 在图像中搜索矩形
        ...
        for obj in tf.classify(net1 , img1, min_scale=1.0, scale_mul=0.5, x_overlap=0.0, y_overlap=0.0):
        ...
        for obj in tf.classify(net2 , img1, min_scale=1.0, scale_mul=0.5, x_overlap=0.0, y_overlap=0.0):
            print("**********\nTop 2 Detections at [x=%d,y=%d,w=%d,h=%d]" % obj.rect())
            sorted_list = sorted(zip(labels, obj.output()), key = lambda x: x[1], reverse = True)
            # 打印准确率最高的结果
            for i in range(1):
                print("%s = %f" % (sorted_list[i][0], sorted_list[i][1]))

        for obj in tf.classify(net3 , img1, min_scale=1.0, scale_mul=0.5, x_overlap=0.0, y_overlap=0.0):
            print("**********\nTop 3 Detections at [x=%d,y=%d,w=%d,h=%d]" % obj.rect())
            sorted_list = sorted(zip(labels, obj.output()), key = lambda x: x[1], reverse = True)
            # 打印准确率最高的结果
            for i in range(1):
                print("%s = %f" % (sorted_list[i][0], sorted_list[i][1]))
