...
| V1 数据集 (~347k samples 含 DAgger) | ✅ 落盘 `runs/sage_pr/full_v5_v3/` (gitignored) |
| SAGE-PR-PushOnly 模型 (PyTorch) | ✅ 完成 (干净架构, 无 inspect/info_gain) |
| BC 训练 (L_policy + 0.3·L_value) | ✅ 完成 |
| DAgger 迭代 | ✅ 通用框架 |
| Rollout search 推理 + `--external-explorer` | ✅ 完成 |
| OpenART 部署 (TFLM + INT8 + QAT) | 🔄 进行中 |

### 🎯 部署架构 (2026-05-13 定型)

**双系统**: 板上 BFS 算法做探索识别 + NN 专心推箱. 两者按"计算复杂度"切, 各取所长:
- **探索阶段** (BFS, O(N·G)): `plan_exploration_v3` 移植 MicroPython 即可,毫秒级
- **推箱阶段** (NN, 1 forward ≤ 50ms): SAGE-PushOnly-Large, 190K params, INT8 ≤ 200KB

### 最新 eval (`v5_push_only`, 全量 1010+ maps, beam=8 lookahead=25, `--external-explorer`)

```
phase 1: 100.00%  (1010/1010)
phase 2:  99.60%  (1006/1010)   失败 4 张
phase 3:  98.91%  (999/1010)    失败 11 张
phase 4:  99.60%  (1007/1011)   失败 4 张
phase 5:  98.71%  (997/1010)    失败 13 张
phase 6:  99.31%  (1004/1011)   失败 7 张
```

**6 phase 全部 ≥ 95%** ✅ — 这是端到端真实部署能力数字 (explorer 跑前置, NN 接管 push). 没有"上帝视角"信息泄漏: 模型只在 post-explorer 状态 (entity 已被 BFS 识别完毕) 上做推箱决策, 不假设自主识别能力. 训练数据本身就是 post-explorer 分布 (`build_dataset_v5._exact_fallback_episode` 先跑 explorer 再产样本).
...
> **§10.1 已解决** (2026-05-13): 采用 "explorer 算法 + NN-for-push" 双系统架构. 模型不再被要求自主探索, info_gain/inspect 相关代码路径全删. 详见 §11 / TODO.md §"新架构任务清单".

### 10.1 ✅ Fully-observed BeliefState 信息泄漏 — 模型不会"探索" (已解决)
...
### 主模型 (推荐用 — 部署架构对齐, 干净 push-only)

```
.agent/sage_pr/runs/v5_push_only/best.pt
```

`v5_push_only` 是 large `SAGEPushOnlyRanker`, **190K params** (INT8 ≤ 200 KB).
- 架构: 输入 X_grid [27, 10, 14] + X_cand [64, 118] + u_global [12] → (score [64], value)
- 训练: 80 epoch from scratch, **不含 inspect / info_gain / deadlock_head 任何探索相关组件**
- 数据: 347k 样本 (6 phase base + 23 DAgger 轮次 + 5 targeted), 全部 post-explorer 状态
- 部署: 板上 BFS 算法跑 `plan_exploration_v3` → NN 接管 push, **不假设上帝视角**

`build_model_from_ckpt()` 自动识别 push_only / full + default / large 4 种组合.

```bash
# 全量 eval --external-explorer (跟部署架构一致)
python experiments/sage_pr/rollout_search_eval.py \
    --ckpt .agent/sage_pr/runs/v5_push_only/best.pt \
    --phases 5 --max-maps 1010 --use-verified-seeds \
    --beam 8 --lookahead 25 --mode v1 --external-explorer

# 看模型在某张图上的可视化 (v1_v3 老师 vs 模型 beam search 对比)
python experiments/sage_pr/preview_trajectory.py \
...
```
.agent/sage_pr/runs/v3_large9/best.pt   # full 194K, 含 info_gain_head 等冗余
```

`v3_large9` 在 fully_observed=True 上帝视角下训练, 不适合直接部署 (依赖视觉模块预识别全部 entity). 留作 push 能力的 baseline 对比.
