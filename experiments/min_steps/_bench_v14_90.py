"""90-map validation of v14 (best teacher candidate)."""
...
from experiments.min_steps.harness import run_planner, planner_v0_godmode_lowerbound
...
for ph in ['phase4', 'phase5', 'phase6']:
    for k, v in list(manifest['phases'][ph].items())[:30]:
...
cfgs = [
    ...
    ('v8 α=0.4', functools.partial(planner_oracle_v8, alpha=0.4)),
    ('v12 α=0.4', functools.partial(planner_oracle_v12, alpha=0.4)),
    ('v14 α=0.4', functools.partial(planner_oracle_v14, alpha=0.4)),
    ('v14 α=0.7', functools.partial(planner_oracle_v14, alpha=0.7)),
    ('v14 α=1.0', functools.partial(planner_oracle_v14, alpha=1.0)),
    ('v14 α=2.0', functools.partial(planner_oracle_v14, alpha=2.0)),
]
for label, pl in cfgs:
    ...
    print(f'  {label:<14}: wins {wins:>3}/{len(trials)}  avg={avg:.2f}  gambling={tg}/{tf}={rate:.1f}%', flush=True)
