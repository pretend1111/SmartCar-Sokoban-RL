"""Planner oracle v10 — log entropy penalty (Bayesian).

vs v9: penalty = α × log2(n_possible) instead of α × (n-1)
当 n=2: log2(2)=1 vs (2-1)=1 (同)
当 n=3: log2(3)≈1.58 vs (3-1)=2 (log 更宽容)
当 n=5: log2(5)≈2.32 vs 4 (log 大幅宽容)

理论意义: 信息论上, n 个可能性 → log2(n) 比特不确定性. 求解器希望惩罚正比于
information gain.
"""

from experiments.min_steps.planner_oracle_v9 import (
    oracle_v9_min_steps, planner_oracle_v9,
)
import math

# v10 = v9 with log penalty. We need to override the penalty calc.
# Easiest: parameterize v9 to take a "penalty_fn" callable.
# But v9 doesn't expose. Let's monkey-import + reuse.

# Actually I'll just call the same code, with a wrapper that scales alpha
# differently. The math: alpha_v10 * log2(n) ≈ alpha_v9 * (n-1)
# For n=3: alpha_v10 * 1.58 = alpha_v9 * 2 → alpha_v10 = 1.27 * alpha_v9
# For n=2: same
# So scaling is roughly constant. But penalty SHAPE differs.

# To properly test v10, let me actually fork v9 with log penalty.
...
def oracle_v10_min_steps(eng_init: GameEngine, plan: Optional[List] = None,
                          *, god_time_limit: float = 30.0,
                          allow_proactive_rotation: bool = True,
                          alpha: float = 1.0,
                          penalize_target: bool = True,
                          ) -> Optional[OracleResult4]:
    """Same as v9 but penalty = alpha * log2(n_possible)."""
    if plan is None:
    ...
    def compute_known(scan_set: int, push_idx: int):
        ...
        if len(known_classes) >= N_box - 1: known_classes = class_universe
        if len(known_nums) >= N_target - 1: known_nums = num_universe
    ...
    parent_act = {init_state: None}
    real_steps = {init_state: 0}
    full_mask = (1 << n_s) - 1 if n_s > 0 else 0

    pq = [(0.0, 0, init_state)]
    tb = 1
    best_cost = float('inf'); best_st = None
...
def planner_oracle_v10(eng: GameEngine, *, alpha: float = 1.0) -> None:
