"""Planner oracle v12 — v8 + bijective propagation.

Sokoban 配对约束: box.class_id ↔ target.num_id 严格 1-1.
所以 known_classes 和 known_nums 应该互相 propagate:
  - 若 all box classes known (N-1 + exclusion), 也意味着 all target nums known
  - 反之亦然

之前 v8/v10 独立追踪两个 set, 错过了这层相互信息.
"""
...
def oracle_v12_min_steps(eng_init: GameEngine, plan: Optional[List] = None,
    ...
    # Bijective: under Sokoban unique pairing, class_universe == num_universe
    ...
    def compute_known(scan_set, push_idx):
        ...
        # v12 新增: bijective propagation
        # 任何 known class_id 对应一个 target.num_id (same value due to bijection)
        # 反之亦然. 所以 known_classes 应包含 known_nums 中的值 (and vice versa).
        merged = known_classes | known_nums
        known_classes = merged
        known_nums = merged
        # 排除律
        if len(known_classes) >= N_box - 1: known_classes = full_id_universe
        if len(known_nums) >= N_target - 1: known_nums = full_id_universe
        # 再次 propagation (若一侧 universe, 另一侧也是)
        merged = known_classes | known_nums
...
def planner_oracle_v12(eng: GameEngine, *, alpha: float = 0.4) -> None:
    ...
    res = oracle_v12_min_steps(clone, plan=plan, alpha=alpha)
    if res is None:
