"""Planner oracle v8 — soft penalty for gambling pushes.

Score function:
  total_cost = real_engine_steps + α × Σ(belief_uncertainty for each gambling push)

每个 push 的 uncertainty:
  - box.class_id ∈ effective_known_classes (含排除律): 0
  - 否则: |possible_class_ids_for_this_box| - 1

target 同样.

DP 优化的是 total_cost. 实际 plan 重放仅算 engine steps.

α 调参: 越大越倾向先扫. 默认 α=1.0.

理论意义: penalty ≈ 期望失败惩罚. 若 α 设为 scan 平均代价, 则:
  - 扫一个 entity: cost K_scan, eliminate 1 bit
  - 赌 N 个未知 box 之一: penalty α*(N-1), 平均成功率 1/N
  - 期望成本相当于 scanning's K_scan
所以 α ≈ K_scan 时两者等价, planner 自然选 mixed strategy.
"""
...
def oracle_v8_min_steps(eng_init: GameEngine, plan: Optional[List] = None,
    ...
    """Soft penalty oracle.
    alpha: penalty multiplier per bit of uncertainty.
    penalize_target: 是否对 target.num_id 未知也加 penalty (默认 yes).
    """
    if plan is None:
    ...
    def compute_known(scan_set: int):
        ...
        known_classes = sc_c | forced_classes_set
        known_nums = sc_n | forced_nums_set
    ...
    # 记每个 state 的 "real steps" (不含 penalty), 重放用
    ...
    while pq:
        ...
        # Push next — with soft penalty (仅首次推该 box 时扣)
        if push_idx < n_p:
    ...
    while cur is not None and parent_act.get(cur) is not None:
        ...
        cur = parent[cur]
    actions.reverse()
    # 返回 real steps (不含 penalty), penalty 只用于 DP 选 plan
    return OracleResult4(cost=real_steps[best_st], interleaving=actions)


def planner_oracle_v8(eng: GameEngine, *, alpha: float = 1.0) -> None:
    ...
    res = oracle_v8_min_steps(clone, plan=plan, alpha=alpha)
    if res is None:
